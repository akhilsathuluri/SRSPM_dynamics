...
double Cmat0802(double* gen_coords, double* dgen_coords)
{
	//Variables
...
cp6=cos(p6);
	...
	double Cmat0802 = 0. + (dp2*(4.2987295703513615*cs2*ss2 - 1.157186018995715*(-0.125*cs2*ss2*pow(cp2,2) - 0.125*cs2*ss2*pow(sp2,2)) - 11.34042298615801*(-2*cs2*ss2*pow(cp2,2)*pow(-0.75 + l2,2) - 2*cs2*ss2*pow(-0.75 + l2,2)*pow(sp2,2)) - 0.20339*(-(cs2*ss2*pow(cp2,2)*pow(l2,2))/18. - (cs2*ss2*pow(l2,2)*pow(sp2,2))/18.)))/2.;
