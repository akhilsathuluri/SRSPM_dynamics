...
double Cmat0214(double* gen_coords, double* dgen_coords)
{
	//Variables
...
cp6=cos(p6);
	...
	double Cmat0214 = 0. + (dp2*(11.34042298615801*(2*(-0.75 + l2)*pow(cp2,2)*pow(cs2,2) + 2*(-0.75 + l2)*pow(cs2,2)*pow(sp2,2)) + 0.20339*((l2*pow(cp2,2)*pow(cs2,2))/18. + (l2*pow(cs2,2)*pow(sp2,2))/18.)))/2.;
